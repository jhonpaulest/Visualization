...
output_dir = r'E:\STMKG\Semester7\Praktik_PCLN\UTS\WRF-NEW\ki\full'
...
for timeidx in range(num_times):
    ...
    # Buat kontur untuk K-Index
    levels = np.arange(0, 45, 1)
    ki_contours = plt.contourf(lons, lats, ki,
                              levels=levels,
                              cmap=get_cmap("RdYlBu_r"),
                              transform=crs.PlateCarree())
    
    # Tambahkan colorbar
    cbar = plt.colorbar(ki_contours, ax=ax, orientation="vertical", pad=.02)
    cbar.set_label('K-Index (°C)', fontsize=10)
    cbar.set_ticks(np.arange(0, 45, 2))
    ...
    plt.title(f"KI {title_time}", pad=7, fontsize=15)
    
    # Simpan gambar
    output_filename = f'ki_{current_datetime.strftime("%Y%m%d_%H%M")}.png'

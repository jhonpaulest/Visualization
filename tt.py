...
output_dir = r'E:\STMKG\Semester7\Praktik_PCLN\UTS\ERA5\tt'
...
for time_idx in range(num_times):
    ...
    tt_time = tt.isel(valid_time=time_idx)
    ...
    # Plot data CAPE
    im = tt_time.plot(ax=ax, 
                       cmap='turbo',
                       vmin=40,
                       vmax=46,
                       cbar_kwargs={'label': 'TT (°C)',
                                    'orientation': 'vertical',
                                    'pad': .02, 
                                    'ticks': [40, 40.5, 41, 41.5, 42, 42.5, 43, 43.5, 44, 44.5, 45, 45.5, 46]})
    ...
    cbar.set_label('TT (°C)', fontsize=5)    # Menyesuaikan ukuran font label
    ...
    # Menambahkan judul
    plt.title(f'TT ERA5 {waktu_valid_str}', pad=3, fontsize=7, weight= 'bold')
    
    # Mengatur layout
    plt.tight_layout()
    
    # Menyimpan plot
    output_filename = f'tt_{pd.to_datetime(waktu_valid).strftime("%Y%m%d_%H%M")}.png'

...
output_dir = r'E:\STMKG\Semester7\Praktik_PCLN\UTS\ERA5\cape'
...
for time_idx in range(num_times):
    ...
    cape_time = cape.isel(valid_time=time_idx)
    ...
    # Plot data CAPE
    im = cape_time.plot(ax=ax, 
                       cmap='turbo',
                       vmin=0,
                       vmax=1400,
                       cbar_kwargs={'label': 'CAPE (J kg$^{-1}$)',
                                    'orientation': 'vertical',
                                    'pad': .02,
                                    'ticks': [0, 100, 200, 300, 400, 500, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400]})
    ...
    cbar.set_label('CAPE (J kg$^{-1}$)', fontsize=5)  # Menyesuaikan ukuran font label
    ...
    gl.xlabel_style = {'size': 5}  # Ukuran font untuk label bujur (longitude)
    gl.ylabel_style = {'size': 5}  # Ukuran font untuk label lintang (latitude)
    
    # Menambahkan judul
    plt.title(f'CAPE ERA5 {waktu_valid_str}', pad=3, fontsize=7, weight= 'bold')
    
    # Mengatur layout
    plt.tight_layout()
    
    # Menyimpan plot
    output_filename = f'cape_{pd.to_datetime(waktu_valid).strftime("%Y%m%d_%H%M")}.png'
